...
This project showcases a **state-of-the-art explainable AI system** that classifies wine reviews using **ModernBERT**, the latest advancement in transformer architecture. The system not only predicts wine quality with high accuracy but also provides **human-interpretable explanations** for its decisions, making it valuable for sommeliers, wine enthusiasts, and businesses.

### 🚀 Technical Details
- **70.2% accuracy** on 169K+ wine reviews with 4-class classification
- **ModernBERT fine-tuning** with advanced optimization techniques
- **Production-ready explainable AI** with Integrated Gradients
- **Interactive web interface** for real-time predictions and explanations
- **Comprehensive Model tracking pipeline** with tensordboard and [huggingface](https://huggingface.co/scbtm/ModernBERT_wine_quality_reviews_ft)

## ✨ Advanced Features

### 🔬 Explainable AI Implementation
- **Integrated Gradients**: Advanced attribution method showing exactly which words influence predictions
- **Subword Token Merging**: Custom algorithm for improved readability of explanations  
...
- **Gradio Web App**: Professional-grade UI for model interaction
...
- **Example Gallery**: Curated examples showcasing model capabilities

### 🏗️ Technical Architecture
- **ModernBERT Base**: Cutting-edge transformer architecture optimized for efficiency
- **Multi-input Processing**: Structured data integration (country, variety, description)
- **Custom Loss Functions**: Optimized for imbalanced wine quality distribution
- **Advanced Training**: Early stopping, warm-up scheduling, gradient clipping

## 🔧 Model Training & Optimization

The model leverages a **comprehensive wine review dataset** from [Kaggle](https://www.kaggle.com/datasets/zynicide/wine-reviews) with sophisticated preprocessing:

- **169K+ wine reviews** across 4 quality categories
- **Structured input format**: "Country: [X]; Variety: [Y]; Description: [Z]"
- **Smart binning strategy**: Points-based quality categorization (≤85: bad, 86-88: average, 89-92: good, >92: excellent)
- **Advanced training techniques**: Learning rate scheduling, gradient norm clipping, early stopping
...
The system provides instant wine quality predictions with visual explanations showing which words most influence the decision:
...
- **PyTorch**: Deep learning framework with CUDA optimization
- **Captum**: Facebook's explainability library for Integrated Gradients
...
3. **Subword Merging**: Custom algorithm for readable explanations
4. **Visualization**: HTML-based color-coded word importance

## 🔍 Understanding Integrated Gradients

**What is Integrated Gradients?**
Integrated Gradients is a method that explains AI model predictions by measuring how much each input word contributes to the final decision. Think of it as asking: *"If I gradually add each word to an empty sentence, how much does each word change the model's prediction?"*
...
**Why This Matters**
- **Trustworthy**: Unlike other methods, Integrated Gradients satisfies mathematical properties that make explanations reliable, i.e. satisfies completeness: the attributions across tokens sum (approximately) to the model’s output difference between input and baseline. That’s why it’s considered principled.
- **Intuitive**: The results match human intuition about which words seem important
- **Comprehensive**: Every word gets a score, showing both positive and negative contributions

**In Our Wine Classifier**
...
- **Wine Industry**: Quality assessment automation for wineries and distributors
- **E-commerce**: Automated product categorization and recommendation systems  
- **Hospitality**: Sommelier decision support and customer experience enhancement
